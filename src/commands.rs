...
pub fn cmd_add(sub_m: &ArgMatches, tasks: &mut Vec<Todo>) {
    ...
    let scheduled_time = NaiveDateTime::parse_from_str(
        sub_m.get_one::<String>("scheduled").unwrap_or(&none),
        "%Y-%m-%d_%H:%M:%S",
    )
    .ok();
    let deadline_time = NaiveDateTime::parse_from_str(
        sub_m.get_one::<String>("deadline").unwrap_or(&none),
        "%Y-%m-%d_%H:%M:%S",
    )
    .ok();
    let creat_time = NaiveDateTime::parse_from_str(
...
pub fn cmd_list(sub_m: &ArgMatches, tasks: &[Todo]) {
    ...
    let scheduled_before = NaiveDate::parse_from_str(
        sub_m.get_one::<String>("scheduled-before").unwrap_or(&none),
        "%Y-%m-%d",
    )
    .ok();
    let scheduled_after = NaiveDate::parse_from_str(
        sub_m.get_one::<String>("scheduled-after").unwrap_or(&none),
        "%Y-%m-%d",
    )
    .ok();
    ...
    let deadline_before = NaiveDate::parse_from_str(
        sub_m.get_one::<String>("deadline-before").unwrap_or(&none),
        "%Y-%m-%d",
    )
    .ok();
    let deadline_after = NaiveDate::parse_from_str(
        sub_m.get_one::<String>("deadline-after").unwrap_or(&none),
        "%Y-%m-%d",
    )
    .ok();
            ...
            for task in tasks {
                ...
                if day_filters != None {
                    if scheduled_at != None
                        ...
                        && task.scheduled_time.map(|d| d.date()) == scheduled_at
                    {
                        s_at = true;
                    }
                    if scheduled_before != None
                        && task.scheduled_time != None
                        && task.scheduled_time.map(|d| d.date()) <= scheduled_before
                    {
                        s_before = true;
                    }
                    if scheduled_after != None
                        && task.scheduled_time != None
                        && task.scheduled_time.map(|d| d.date()) >= scheduled_after
                    {
                        s_after = true;
                    }
                    if deadline_at != None
                        && task.deadline_time != None
                        && task.deadline_time.map(|d| d.date()) == deadline_at
                    {
                        d_at = true;
                    }
                    if deadline_before != None
                        && task.deadline_time != None
                        && task.deadline_time.map(|d| d.date()) <= deadline_before
                    {
                        d_before = true;
                    }
                    if deadline_after != None
                        && task.deadline_time != None
                        && task.deadline_time.map(|d| d.date()) >= deadline_after
                    {
                        d_after = true;
                    }
                }
...
pub fn cmd_edit(sub_m: &ArgMatches, tasks: &mut Vec<Todo>) {
    ...
    let creat_time = NaiveDateTime::parse_from_str(
        sub_m.get_one::<String>("creat").unwrap_or(&none),
        "%Y-%m-%d_%H:%M:%S",
    )
    .ok();
    let start_time = NaiveDateTime::parse_from_str(
        sub_m.get_one::<String>("start").unwrap_or(&none),
        "%Y-%m-%d_%H:%M:%S",
    )
    .ok();
    let finish_time = NaiveDateTime::parse_from_str(
        sub_m.get_one::<String>("finish").unwrap_or(&none),
        "%Y-%m-%d_%H:%M:%S",
    )
    .ok();
    let scheduled_time = NaiveDateTime::parse_from_str(
